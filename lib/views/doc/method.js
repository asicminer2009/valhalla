...
export default class Method extends EtchComponent {
  hint () {
    ...
    } else if (this.props.method.name === 'contains' &&
    this.props.method.parameters.length === 1 &&
    (this.props.method.returnType.raw === 'bool' || this.props.method.returnType.raw === 'global::bool') &&
    !this.props.method.isStatic) {
      return <p>
        <span className='icon icon-light-bulb'></span>
        You can use <code>x in y</code> instead of <code>y.contains (x)</code>
      </p>
    }
  }
  ...
  render () {
    return <div>
      <h1><Icon item={this.props.method} /> {this.props.method.name}<span className='text-subtle'> &mdash; {this.props.method.qualifiedName}</span></h1>
      <div className='block'>
