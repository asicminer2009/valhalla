...
export default class ValaProvider {
  constructor () {
    ...
    this.excludeLowerPriority = atom.config.get('valhalla.removeDefaultSuggestions')
  ...
  */
    ...
    if (symb.name && symb.type !== 'creation-method' && ((symb.type !== 'method' && symb.type !== 'property') || symb.isStatic || forInstance)) {
      if (symb.qualifiedName.startsWith(this.context.longPrefix) ||
        ...
        for (const usn of this.context.usings) {
          if (symb.qualifiedName.startsWith([usn.name, this.context.prefix].join('.')) && symb.name.startsWith(this.context.prefix)) {
            this.suggestions.push({
  ...
  async suggestUsings (symb) {
    if (symb.name && !this.context.usings.includes(symb.qualifiedName) && symb.type === 'namespace' &&
       (symb.qualifiedName.startsWith(this.context.longPrefix) || this.context.prefix === ' ')) {
      this.suggestions.push({
        text: `${symb.qualifiedName};`,
    ...
    // We can only find namespaces in namespaces and in the root (which has usings)
    return symb.type === 'namespace' || symb.usings !== undefined
  }
