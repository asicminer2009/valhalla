...
export default class ValaProvider {
    constructor(serializedState) {
        ...
        this.re = {
            ns: /namespace (\S*)/,
            using: /^using /,
            cls: /(public |private |internal )class (\S*) : (\S*)( ?{)?/,
            isCls: / ?class /,
            ...
            let explore = (scope) => {
                ...
                if (scope.vapi && nsMatch && line.match(this.re.using) && nsMatch[1].match(prefix)) {
                    ...
                    while (parent.top) {
                        ...
                        let topMatch = parent.top.name.match(nsthis.re.ns);
                        if (topMatch) {
                    ...
                    scope.children.forEach((child) => {
                        if (!haveSubNs && child.name.match(this.ns.re)) haveSubNs = true;
                    });
                ...
                if (scope.file == editor.getPath() && scope.at[0][0] <= bufferPosition.row && scope.at[1][0] >= bufferPosition.row + 1) {
                    ...
                    scope.vars.forEach((localVar) => {
                        ...
                        if (localVar.name.match(_prefix) && !line.endsWith('.' + prefix)) {
                            let sugg = {
                                text: localVar.name,
                                type: 'variable',
