    ...
    getSuggestions ({editor, bufferPosition, scopeDescriptor, prefix, activatedManually}) {
        ...
        return new Promise ((resolve) => {
            ...
            let nsRe = /namespace (\S*)/,
                ...
                memberRe = /(public|private|internal) (\w+) (.+)/,
                parRe = /\(.*\)/,
                dotRe = /\./;
            ...
            let explore = (scope) => {
                ...
                if (scope.vapi && nsMatch && line.match(usingRe) && nsMatch[1].match(prefix)) {
                    let suggestion = {
                        text: nsMatch[1] + ';',
                        type: 'import',
                ...
                if (newMatch) {
                    ...
                    let clsMatch = scope.name.match(clsRe)
                ...
                if (scope.file == editor.getPath() && scope.at[0][0] <= bufferPosition.row && scope.at[1][0] >= bufferPosition.row + 1) {
                    if (!thisScope && scope.name.match(isClsRe)) {
                        thisScope = scope;
                        topScope = scope;
                    }
                    ...
                    scope.vars.forEach((localVar) => {
                        ...
                        let _prefix = prefix.replace(dotRe, '\\.');
                ...
                if (thisScope) {
                    thisScope.children.forEach((child) => {
                        let memberMatch = child.name.match(memberRe);
                ...
                if (topScope/*suggestionType.includes('local')*/) {
                    topScope.children.forEach((scope) => {
                        ...
                        if (methodMatch) {
                            ...
                            if (!scope.top.name.match(isClsRe)) {
