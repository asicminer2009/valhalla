...
export default class ValaProvider {
    ...
    getSuggestions({editor, bufferPosition, scopeDescriptor, prefix, activatedManually}) {
        ...
        return new Promise((resolve) => {
            ...
            // matches
            const usingMatch = line.match(this.re.using);
            const newMatch = line.match(this.re.newInstance);
            ...
            const shouldSuggestClasses = trimLine[0] === trimLine[0].toUpperCase() && trimLine === prefix;
            const shouldSuggestStructs = trimLine === prefix;
            ...
            for (const snippet of snips) {
                if (snippet.name.includes(prefix) || snippet.prefix.includes(prefix)) {
                    suggestions.push({
