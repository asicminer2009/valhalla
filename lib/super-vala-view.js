...
export default class SuperValaView {
    ...
    constructor(serializedState, type) {
        if (!type) type = 'class';
        ...
        message.textContent = `Please enter the name of your new ${type}.`;
        ...
        input.addEventListener('keyup', (event) => {
            ...
            // esc
            if (event.keyCode == 27) {
                atom.commands.dispatch(input, 'super-vala:new-' + type); // toggle the modal panel
            }

            //enter
            if (event.keyCode == 13) {
                new Generator().gen(input.getModel().getText(), type);
                atom.commands.dispatch(input, 'super-vala:new-' + type); // toggle the modal panel
            }
        });
