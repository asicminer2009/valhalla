...
export default class ValaParser {
  ...
  */
  error (msg) {
    try {
      ...
      report.error(msg, 0, [0, 1], this.path)
    }
  }
  ...
  */
  ...
  parseStructDecl (parent) {
    ...
    str.position.end = this.position
    this.expect('right-brace')
  ...
  parseIfaceDecl (parent) {
    ...
    iface.position.end = this.position
  ...
  parseDelegateDecl (parent) {
    ...
    dlg.position.end = this.position
  ...
  parseSignalDecl (parent) {
    ...
    sig.position.end = this.position
    this.expect('semi-colon')
  ...
  parseEnum (enm) {
    ...
    while (true) {
      if (this.current.type === 'identifier' || this.current.type === 'at' || this.current.type === 'integer-literal') {
        ...
        val.position.end = this.position
        enm.position.end = this.position // Will be updated after if the enum doesn't ends here
        if (this.accept('comma')) {
          enm.children.push(val)
          enm.position.end = this.position
  ...
  parseField (parent) {
    ...
    fld.position.end = this.position
    this.expect('semi-colon')
  ...
  parsePropertyDecl (parent, doc) {
    ...
    prop.position.end = this.position
    this.expect('right-brace')
  ...
  parseMethodDecl (parent, doc) {
    ...
    meth.position.end = this.position
  ...
  parseConstructorDecl (parent) {
    ...
    ctor.position.end = this.position
